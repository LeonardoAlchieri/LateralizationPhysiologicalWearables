...
from numpy import nan, ndarray, asarray
...
def prepare_temporal_consistent_idxs(
    x: ndarray,
    y: ndarray,
    groups: ndarray,
    test_percentage: float = 1 / 3,
    n_folds: int = 5,
) -> tuple[list, list]:
    if len(x) != len(y):
        raise RuntimeError(
            f"x and y must have the same length. Recieved {len(x)} and {len(y)}"
        )
    if len(x) != len(groups):
        raise RuntimeError(
            f"x and groups must have the same length. Recieved {len(x)} and {len(groups)}"
        )
    ...
    subgroups = [get_train_test_folds(groups=unique_groups, fold_size=fold_size) for _ in range(n_folds)]
    compelte_idx_list = asarray(list(range(len(x))))
    cv_folds = []
    for train_group, test_group in subgroups:
        train_idx = []
        test_idx = []
        for group in test_group:
            current_group_idx = groups == group
            current_idx = compelte_idx_list[current_group_idx]
            current_y = y[current_group_idx]

            positive_idx = current_y == 1
            negative_idx = current_y == 0

            current_idx_positive = current_idx[positive_idx]
            current_idx_negative = current_idx[negative_idx]

            current_total_len_positive = len(current_idx_positive)
            train_idx.extend(
                current_idx_positive[
                    : int(current_total_len_positive * (1 - test_percentage))
                ]
            )
            test_idx.extend(
                current_idx_positive[
            ...
            current_total_len_negative = len(current_idx_negative)

            train_idx.extend(
                current_idx_negative[
                    : int(current_total_len_negative * (1 - test_percentage))
                ]
            )
            test_idx.extend(
                current_idx_negative[
        ...
        train_idx.extend(
            [idx for idx in compelte_idx_list if groups[idx] in train_group]
        )
...
def run_nested_cross_validation_prediction(
    ...
    **kwargs
    ...
    random_states_classifiers = randint(0, 10000, n_seeds_to_test_classifiers)
