...
from src.ml.classifier_list import CLASSIFIERS_HYPERPARAMETER_LIST_FEATURE_SELECTION
...
def perform_grid_search_estimation(
    ...
    return (
        classifier.__name__,
        acc,
        result.steps[1][1].best_estimator_.feature_importances_,
    )
...
def fit_with_hyperparameters(
    ...
    models: list[tuple[str, float]] = [
        ...
        for classifier, search_space in CLASSIFIERS_HYPERPARAMETER_LIST_FEATURE_SELECTION.items()
    ]
    models: dict[str, float] = {
        k: [acc, *feature_importance] for k, acc, feature_importance in models
    }

    num_features: int = x_train.shape[1]
    models = DataFrame.from_dict(
        models,
        orient="index",
        columns=["Balanced Accuracy", *[f"FI, feat#{i}" for i in range(num_features)]],
    )
    return models
