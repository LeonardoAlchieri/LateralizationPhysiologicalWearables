...
from src.ml.nested_partial_temporal_consistent import (
    run_nested_cross_validation_prediction,
    run_opposite_side_prediction_hyper_updated,
)
...
parser.add_argument(
    ...
    default="src/run/ml/config_nested_partial_temporal_consistency.yml",
)
...
def main():
    ...
    # else:
    #     features_left_opposite = features_left
    #     features_right_opposite = features_right
    ...
    # FIXME: For the moment I have removed the opposite side training, since I'm not sure if it makes sense to do it in this Temporal consistent paradigm.
    # print("\n")
    ...
    # bad_sides = list(product(
    #     ["left", "right", "diff"],
    #     ["left+right", "left+diff", "right+diff", "left+right+diff"],
    # ))
    # bad_sides.extend(list(product(
    #     ["left+right", "left+diff", "right+diff", "left+right+diff"],
    #     ["left", "right", "diff"],
    # )))
    # bad_sides.extend(list(product(
    #     [el for el in complete_data.keys() if el != "left+right+diff"], ["left+right+diff"]
    # )))
    # bad_sides.extend(list(product(
    #      ["left+right+diff"], [el for el in complete_data.keys() if el != "left+right+diff"]
    # )))
    ...
    #     )
    # ):
    #     pbar.set_description(f"Opposite side progress (current {opposite_side})")
